...
% Description:
%
% Unconstrained maximum entropy VAR modeling via the Vieira-Morf lattice
% filter algorithm [2] / Partial Correlation Estimation;
% In [1] this mode was used and (incorrectly) denominated as Nutall-Strand.
%
% This algorithm is a good choice when you have substantially more data
% than parameters to fit.
%
% Author Credits:
%
% This implementation is based on the mvar() function from Alois Schloegl's
% TSA toolbox [4]
%
% References and Code:
%
% [1] A. Schloegl, Comparison of Multivariate Autoregressive Estimators.
%     Signal processing, Elsevier B.V. doi:10.1016/j.sigpro.2005.11.007
% [2] S.L. Marple. Digital Spectral Analysis with Applications. Prentice Hall, 1987
% [3] http://pub.ist.ac.at/~schloegl/matlab/tsa/
%
% ------------------------------------------------------------------------
% INPUT:
...
PEF = PE(:,1:M);
PEB = PE(:,1:M);
if true % match indentation in mvar implementation in the TSA toolbox
    for K = 1:morder
...
if any(ARF(:) == inf)
    ...
    p = 3;
    FLAG_MATRIX_DIVISION_ERROR = ~all(all(isnan(zeros(p)/zeros(p)))) | ~all(all(isnan(nan(p)/nan(p))));
